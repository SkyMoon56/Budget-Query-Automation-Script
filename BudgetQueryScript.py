...
def get_period_and_month():
    ...
    Fiscal year starts in July (July = P01, August = P02 ... June = P12).
    Example: Running on April 3 -> P09 MARCH 2026
    """
    today = date.today()
    first_of_this_month = today.replace(day=1)
    last_month = first_of_this_month - timedelta(days=1)
    period = ((last_month.month - 7) % 12) + 1  # July = P01, August = P02 ... June = P12
    period_str = f"P{period:02d}"  # Zero-padded: P01, P02 ... P12
    month_name = last_month.strftime("%B").upper()
...
def write_workbook(df, department, period_str, month_name, year):
    ...
    Formatting applied:
      - A1: "Org Budget Inquiry" label -- gray header bar to match Cash Recon styling
    ...
    # Gray header bar fill -- matches PeopleSoft export reference (C8C8C8)
    GRAY_FILL = PatternFill("solid", start_color="C8C8C8")
    ...
    # A1: Static label matching the reference output -- gray fill like PeopleSoft export
    ws["A1"] = "Org Budget Inquiry"
    ws["A1"].font = GRAY_FONT
    ws["A1"].fill = GRAY_FILL
    ws["A1"].border = DOUBLE_BORDER
    ...
    # Row 2: Bold column headers with gray bar fill and double border on all sides
    ...
    # Auto-filter on row 2 across all columns, spanning through the last data row
    ws.auto_filter.ref = (
        ws.cell(row=2, column=1).coordinate
        + ":"
        + ws.cell(row=ws.max_row, column=len(headers)).coordinate
    )
...
def main():
    ...
    # Drop BusUnit column -- not needed in the final output
    combined = combined.drop(columns=["BusUnit"], errors="ignore")

    # Force Unit column to always read NORMN
